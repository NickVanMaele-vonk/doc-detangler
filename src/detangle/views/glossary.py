...
BANNER = """\
<!--
GENERATED by `detangle generate` — DO NOT HAND-EDIT.

Derived from the canonical concept records (`concepts/*.yaml`) and the
canonical cycle register (`registers/cycles.yaml`), ordered by the concept
graph. D9 makes the record the definition's only home; this file is a view of
it. To change an entry, edit its record and regenerate — `detangle generate
--check` compares bytes and fails on a hand-edit.
...
OVERVIEW_GAP = """\
<!-- gap:overview -->
> **Gap — the overview is not written.** `detangle generate` emits no domain
> prose: every substantive word in this file traces to a concept record, and
> an overview of the domain would be invented text (C2, criterion 7). A human
> writes it — but not into this block, which is regenerated. Where that text
> is to live canonically is an open decision.
"""
...
def build(cg: ConceptGraph, rel: str = "glossary.md") -> Glossary:
    ...
    findings = [
        warn(
            "overview-gap",
            f"{rel}:overview",
            "the glossary has no overview; the generator will not write domain "
            "prose (C2), so the marked gap stands until a human writes one. It "
            "cannot be written into this file, which is regenerated — the "
            "canonical home for that text is an open decision",
        )
    ]
