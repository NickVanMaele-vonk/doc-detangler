...
| OpenSanctions | Open-source sanctions, regulatory enforcement, and financial crime database (corpus: MCL 1.0 Layer 3) | OpenSanctions (opensanctions.org) | M only | §1.0 Layer 3 feed: participant LEI lookup for RRF enrichment and the HSL_prior_stor proxy — see `regulatory-risk-factor`. Corpus caution: Belgian-specific FSMA enforcement actions may not be present. External vendor, routed here per criterion 3 from the §3 M bulk (Eurex precedent; Nick's ruling 2026-07-30). |
| OpenCorporates | Open database of global company registrations, corporate structures, beneficial ownership (where public), and registered addresses (corpus: MCL 1.0 Layer 3) | OpenCorporates Ltd | M only | §1.0 Layer 3 feed: participant entity mapping; counterparty relationship mapping where BT-06 or PI09 is confirmed — see `bt-06`, `pi09`. Commercial API licence required (corpus limitation note). External vendor, routed here per criterion 3 (Nick's ruling 2026-07-30). |
| Azure OpenAI / AWS Bedrock | Large Language Model API for automated analytical narrative generation and analyst augmentation (corpus: MCL 1.0 Layer 4) | Microsoft / Amazon Web Services | M only | Corpus: "Azure OpenAI (GPT-4 class models) or AWS Bedrock (Claude class models)"; "Azure OpenAI (EU data residency) preferred for GDPR compliance". The governance boundary is project content and lives in `automated-narrative-generation` and `ai-governance-prohibitions`. External vendor platforms, routed here per criterion 3 (Nick's ruling 2026-07-30). |
| EuroMTS | — (electronic trading venue for European sovereign and quasi-government bonds) | MTS S.p.A. group | M only | Named in MCL §1.0.1 row 3 as an out-of-perimeter venue whose feed is formally removed — see `cross-venue-surveillance-feed`. External venue, routed here per criterion 3 (Eurex precedent; Nick's ruling 2026-07-30). Distinct from `mts-associated-markets` and `mts-spa`; the corpus never states the group relationship. |
| Tradeweb | — (electronic fixed income trading venue) | Tradeweb Markets Inc. | M only | Named in MCL §1.0.1 row 3 as an out-of-perimeter venue — see `cross-venue-surveillance-feed`. External venue, routed here per criterion 3 (Eurex precedent; Nick's ruling 2026-07-30). |
| Bloomberg | — (financial data, analytics, and trading platform provider) | Bloomberg L.P. | M only | Named in MCL §1.0.1 row 3 as an out-of-perimeter venue or reporting platform — see `cross-venue-surveillance-feed`. The corpus does not say which Bloomberg product is meant. External vendor, routed here per criterion 3 (Eurex precedent; Nick's ruling 2026-07-30). |
| Azure AI Search | Vector search and full-text search index for the analytical knowledge base (corpus: MCL 1.0 Layer 4) | Microsoft | M only | Analyst query interface and STOR precedent retrieval. Corpus data-classification note: the index "must be deployed within the institution's secure environment, not a shared or public search instance". External vendor product, routed here per criterion 3 (Nick's ruling 2026-07-30). |
