...
1. **Step 3.6 (`index.md`)** — unblocked 2026-08-10: all three bodies are
   registered in `detangle.toml [bodies]`, so the generator can now be
   built. Step 3.7 is complete as of the same date — extraction was built
   2026-08-08 and the set-wide dead-entry sweep runs with the full set,
   reported by `detangle graph` as `dead_entries`.
...
4. **Phase 6.2** does not close until `param-low-confidence-threshold` is
   ...
   which is B-9's first scored run (ADR-003 Decision 7 as amended,
   2026-08-10). The deterministic dry-run is done; its report is
   `eval/verify-U.md`.
...
fabrication check would read as a proof it never produced. Decision 6 is
ruled and built (2026-08-10): the seeded-error tests plant a deleted, a
weakened and an invented claim into a copy of the golden, with the
invented-claim limb held as a strict xfail — the done-when's invented-claim
limb cannot be met until B-9 lands. Decision 7 is ruled, amended
(2026-08-10): the `U` dry-run ran deterministically and its report is
committed as `eval/verify-U.md` — the step 7.5 baseline — while the
threshold re-baseline and 6.2's closure moved to B-9's first scored run,
because the deterministic build produces no confidence distribution to
re-baseline against.

| Step | Description |
|------|-------------|
| 7.1 | **Claim decomposition:** split the **detangle set** into atomic claims (LLM-assisted). Reference documents are never decomposed — their claims are not required in the output (C1, 2026-08-05). The LLM assistance is a workflow that produces a PR, not a runtime dependency (ADR-003 Decision 1): a span the deterministic decomposer will not confidently split is flagged, and the approved split lands in `registers/claim-splits.yaml` — home ruled by Nick 2026-08-07 — which `detangle verify` executes. Empty today: 44 flags on `U`, none ruled — they await Nick's dispositions with no scheduled moment (ruled 2026-08-10; an empty register against live flags is the register working). The Decision 7 dry-run ran with zero overrides and dispositioned nothing. Overrides apply to the source only; how a split reaches the restructured output is open (backlog B-10). |
